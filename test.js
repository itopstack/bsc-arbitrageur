...
var log_file = fs.createWriteStream(__dirname + "/log_arbitrage_test.txt", {
  flags: "w",
});
...
const init = async () => {
  ...
  const transactionSender = TransactionSender.factory(
    process.env.WSS_BLOCKS_TEST.split(",")
  );
  ...
  const onBlock = async (block, web3, provider) => {
    ...
    pairs.forEach((pair) => {
      calls.push(async () => {
        ...
        console.log(
          `[${block.number}] [${new Date().toLocaleString()}]: [${provider}] [${
            pair.name
        ...
        if (profit > 0) {
          console.log(
            `[${
              ...
              profit / 1e18
            ).toFixed(3)} $${profitUsd.toFixed(2)} - ${percentage.toFixed(2)}%`
          );
          ...
          const myGasPrice = new BigNumber(gasPrice)
            .plus(gasPrice * process.env.GAS_MULTIPLIER)
            .toString();
          const txCostBNB =
            Web3.utils.toBN(estimateGas) * Web3.utils.toBN(myGasPrice);

          let gasCostUsd =
            (txCostBNB / 1e18) * prices[process.env.BNB_MAINNET.toLowerCase()];
          ...
          if (profitMinusFeeInUsd >= EFFECTIVE_PROFIT) {
            ...
            console.log(
              `[${
                block.number
              }] [${new Date().toLocaleString()}] [${provider}]: sending transactions...`,
    ...
    if (block.number % 40 === 0) {
      console.log(
        `[${
          block.number
        }] [${new Date().toLocaleString()}]: alive (${provider}) - took ${number.toFixed(
          2
        )} ms`
      );
    }
